...
- 样本复核（无效 / 重复 / 待确认）与版本固化、导出
- `QA` 协议

**不做（后续里程碑）**：
- **回流（§6）与 badcase 挖掘（§7）**——本文档先定设计，P0 不实现。
  表结构暂不加 `lineage` / `review` 字段，等实现时用模块自己的迁移补（表所有权约定允许）。
...
2. 一个数据集版本**允许混合** `task_shape` 与 `protocol`。比较时按维度下钻，不假设同质。
3. 跨租户样本**可以直接共用**——不做额外的隔离确认。
   代价是跨租户评测时要注意样本来源，回报是复用率高、回归集不必按租户复制。
4. 回流标注走 **LLM 起草 + 人工确认**；未确认的草稿不得进入 `purpose=gate` 的数据集。
   （**P0 不实现**，见 §9）
5. badcase 挖掘由**机器挖 + 人把关**，挖掘 Agent 只能写草稿，不能复核、不能晋级。
   （**P0 不实现**，见 §9）

**暂缓（先记录想法）**：

1. 每个 cluster 的抽样上限（默认 3）是否按 `purpose` 区分——门禁数据集可能希望
   每个根因只留 1 条，监控数据集可以多留。等回流落地时再定。
2. 混合 `task_shape` 后的**比较口径**：分数按整体算，还是按 `task_shape` / `protocol` 分组算？
   如果混着算，agentic 任务的长尾会淹没单轮问答的表现。建议**默认分组展示、整体作为参考**。
